...
export function processResumenData(movimientos, arqueos, egresos, recaudaciones, filterCaja) {
  ...
  movimientos.filter(m => m.tipo === 'deposito-inversiones' || m.tipo === 'inversion-retiro').forEach(m => {
  ...
  const filteredRows = Object.values(datosPorClave).filter(d => d.nombreCaja !== 'Tesoreria');
