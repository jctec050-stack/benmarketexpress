...
export function processResumenData(movimientos, arqueos, egresos, recaudaciones, filterCaja) {
  ...
  egresos.forEach(e => {
    ...
    // Group card and transfer collections under Administrative Expenses
  ...
  movimientos.forEach(m => {
    ...
    if (!isIngreso) {
      ...
      if (m.tipo === 'operacion' || cat === 'Deposito/Retiro bancario') {
        ...
        const identifier = m.referencia ? `REF: ${m.referencia}` : (m.id ? `ID: ${String(m.id).slice(0, 4)}` : '');
        cat = `DEP. BOCAS - ${m.descripcion || 'S/D'}${identifier ? ` (${identifier})` : ''}`;
  ...
  movimientos.forEach(m => {
    ...
    } else if (m.tipo === 'inversion-retiro') {
      ingresosOtros.inversionRetiro += Math.abs(m.monto || 0);
    } else if (m.tipo === 'sobrante' || (m.tipo === 'operacion' && m.categoria === 'SOBRANTE DE CAJA')) {
      ingresosOtros.sobrantes += Math.abs(m.monto || 0);
    }
  });
  ...
  arqueos.forEach(a => {
    ...
    // Foreign currency tracking for totalAEntregar
    let monGs = 0;
    const mon = a.monedasExtranjeras || {
        usd: a.dolares || { montoGs: 0 },
        brl: a.reales || { montoGs: 0 },
        ars: a.pesos || { montoGs: 0 }
    };
    ['usd', 'brl', 'ars'].forEach(curr => {
        monGs += (mon[curr]?.montoGs || 0);
    });
    d.monedasExtranjerasGs += monGs;
        // Services
      let serv = 0;
      if (a.servicios) {
  ...
  movimientos.forEach(m => {
    ...
    if (m.valorVenta > 0) {
        ...
        if (mon) {
            ['usd','brl','ars'].forEach(curr => {
                const monto = (mon[curr]?.montoGs || (mon[curr]?.cantidad * mon[curr]?.cotizacion) || 0);
                cash += monto;
                d.monedasExtranjerasGs += monto;
            });
        }
    }
  ...
  // Filter out Tesoreria from the Recaudacion table as requested
  // Also omit rows where ingresoTiendaCalculado is 0 (no movements beyond possibly fondo fijo)
  const filteredRows = Object.values(datosPorClave).filter(d => 
    d.nombreCaja !== 'Tesoreria' && d.ingresoTiendaCalculado !== 0
  );
  ...
  filteredRows.forEach(d => {
    ...
    // Final Cash to Deliver (Billetes Gs)
    d.totalAEntregar = d.efectivoBruto - d.monedasExtranjerasGs - d.fondoFijo;
    ...
    sumRecaudadoReal += (d.recaudadoReal || 0);
    sumIngresoTiendaCalculado += (d.ingresoTiendaCalculado || 0);
    sumSobrante += (d.sobrante || 0);
  });
  ...
  return {
    metrics: {
        ...
        totalVentasTienda: sumRecaudadoReal + totalTarjeta + totalPedidosYa + totalCredito + sumSobrante
    },
    summaryData: {
    ...
    tableData: filteredRows.sort((a,b) => a.nombreCajero.localeCompare(b.nombreCajero))
  };
}
