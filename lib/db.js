...
export const db = {
  ...
  async saveEgreso(egreso) {
    ...
    if (egreso.id && egreso.motivoEdicion) {
      const { data: oldData } = await supabase.from('egresos_caja').select('*').eq('id', egreso.id).single();
      if (oldData) {
        const historyEntry = {
  ...
  async saveMovimiento(movimiento) {
    ...
    if (movimiento.id && movimiento.motivoEdicion) {
      const { data: oldData } = await supabase.from('movimientos').select('*').eq('id', movimiento.id).single();
      if (oldData) {
        const historyEntry = {
          fecha: new Date().toISOString(),
  ...
  async saveMovimientoTemporal(item) {
    ...
    if (item.id && item.motivoEdicion) {
      const { data: oldData } = await supabase.from('movimientos_temporales').select('*').eq('id', item.id).single();
      if (oldData) {
        const historyEntry = {
          fecha: new Date().toISOString(),
