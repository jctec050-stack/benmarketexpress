...
const mapMovimientoTemporalToDb = (item) => {
  return {
    ...
    creditoDetalles: item?.creditoDetalles || item?.credito_detalles || null
  }
}
...
const mapMovimientoTemporalToApp = (dbItem) => {
  ...
  return {
    ...
    creditoDetalles: dbItem.creditoDetalles ?? dbItem.credito_detalles ?? null
  }
}
...
const mapEgresoToDb = (eg) => {
  return {
    ...
    moneda: eg?.moneda || 'gs',
    historialEdiciones: eg?.historialEdiciones || eg?.historial_ediciones || []
  }
}
...
export const db = {
  ...
  async saveEgreso(egreso, editorUser = null) {
    // Logica de registro de cambios
    if (egreso.id && egreso.motivoEdicion) {
      const { data: oldDataList } = await supabase.from('egresos_caja').select('*').eq('id', egreso.id).limit(1);
      const oldData = oldDataList?.[0];
      if (oldData) {
        const historyEntry = {
          fecha: new Date().toISOString(),
          usuario: editorUser || egreso.cajero || 'Desconocido',
          motivo: egreso.motivoEdicion,
          datosAnteriores: oldData
        };
        const currentHistory = oldData.historialEdiciones || [];
        egreso.historialEdiciones = [...currentHistory, historyEntry];
      }
    }
  ...
  async saveMovimiento(movimiento, editorUser = null) {
    // Audit log logic
    if (movimiento.id && movimiento.motivoEdicion) {
      const { data: oldDataList } = await supabase.from('movimientos').select('*').eq('id', movimiento.id).limit(1);
      const oldData = oldDataList?.[0];
      if (oldData) {
        const historyEntry = {
          fecha: new Date().toISOString(),
          usuario: editorUser || movimiento.cajero || 'Desconocido',
          motivo: movimiento.motivoEdicion,
          datosAnteriores: oldData
        };
        const currentHistory = oldData.historialEdiciones || [];
        movimiento.historialEdiciones = [...currentHistory, historyEntry];
      }
    }
  ...
  async saveMovimientoTemporal(item, editorUser = null) {
    // Logica de registro de cambios
    if (item.id && item.motivoEdicion) {
      const { data: oldDataList } = await supabase.from('movimientos_temporales').select('*').eq('id', item.id).limit(1);
      const oldData = oldDataList?.[0];
      if (oldData) {
        const historyEntry = {
          fecha: new Date().toISOString(),
          usuario: editorUser || item.cajero || 'Desconocido',
          motivo: item.motivoEdicion,
          datosAnteriores: oldData
        };
        const currentHistory = oldData.historialEdiciones || [];
        item.historialEdiciones = [...currentHistory, historyEntry];
      }
    }
    ...
    if (isOffline) {
      ...
      localDb.upsert('movimientos_temporales', offlineItem)
      return { success: true, offline: true, error: error.message }
    }
  ...
  async obtenerRecaudacion(fecha, cajero = null, caja = null) {
    ...
    if (caja && caja !== 'Todas las cajas') query = query.eq('caja', caja);
  ...
  async obtenerRecaudacionPorRango(fechaDesde, fechaHasta, caja = null) {
    let query = supabase
      .from('recaudacion')
      .select('*')
    ...
    const filtered = all.filter(r =>
      r.fecha >= fechaDesde &&
      r.fecha <= fechaHasta &&
      (!caja || caja === 'Todas las cajas' || caja === 'Todas las Cajas' || r.caja === caja)
    );
    return filtered;
  },
  ...
  async getTotalGeneral(fecha, caja) {
    ...
    return { success: !error, data: data?.[0] || null };
  },

  async saveTotalGeneral(fecha, caja, total, efectivoReal = 0, efectivoRealDetalle = {}) {
    const { data, error } = await supabase
      .from('total_general')
      .upsert({
        fecha,
        ...
        efectivo_real_detalle: efectivoRealDetalle,
        updated_at: new Date().toISOString()
  ...
  async getFondoFijo(caja) {
    ...
    return { success: true, data: data?.[0]?.fondo_fijo || 0 };
  },

  async setFondoFijo(caja, monto, cajero = 'Sistema') {
  ...
  async deleteArqueo(id, fecha, caja) {
    if (!id || !fecha || !caja) return { success: false, error: 'Faltan parámetros' };

    try {
      // 1. Revertir el estado de los movimientos relacionados
      await this.updateRelatedMovementsStatus(fecha, caja, false);

      // 2. Eliminar el arqueo
      const { error } = await supabase
        .from('arqueos')
        .delete()
        .eq('id', id);
      ...
      console.error('Error al eliminar arqueo:', err);
