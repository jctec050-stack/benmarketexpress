...
export function calcularTotalesArqueo(movimientos) {
  const totals = {
    ...
    servicios: {
      ...
      practipago: { lotes: [], monto: 0, tarjeta: 0 },
      otros: {}
    }
  };
  ...
  movimientos.forEach(mov => {
    ...
    // Standard services
    ['apLote', 'aquiPago', 'expressLote', 'wepa', 'pasajeNsa', 'encomiendaNsa', 'apostala', 'practipago'].forEach(key => {
      if (mov.servicios) processService(key, mov.servicios[key]);
    });

    if (mov.servicios) {
      const standardKeys = new Set(['apLote', 'aquiPago', 'expressLote', 'wepa', 'pasajeNsa', 'encomiendaNsa', 'apostala', 'practipago'])
