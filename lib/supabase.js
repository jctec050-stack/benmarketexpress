...
const customStorage = {
  ...
  setItem: (key, value) => {
    try {
      ...
      // Only apply this hack for Supermium or Windows 7/8 to avoid affecting modern browsers
      const isLegacyBrowser = typeof window !== 'undefined' && 
        (window.navigator.userAgent.includes('Windows NT 6.1') || 
         window.navigator.userAgent.includes('Windows NT 6.2') || 
         window.navigator.userAgent.includes('Supermium'));

      if (isLegacyBrowser && key.includes('supabase.auth.token') && typeof value === 'string') {
        try {
